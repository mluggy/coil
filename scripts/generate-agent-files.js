...
agents.push(`| Native MCP tool calls | \`POST ${SITE}/mcp\` (Streamable HTTP, JSON-RPC 2.0) — requires \`Authorization: Bearer <token>\` |`);
agents.push(`| MCP server preview before connect | \`GET ${SITE}/.well-known/mcp/server-card.json\` (public) |`);
agents.push("");
agents.push("## MCP authentication");
agents.push("");
agents.push(`The MCP POST endpoint (\`${SITE}/mcp\`) requires an \`Authorization: Bearer <token>\` header per RFC 6750. The server accepts:`);
agents.push("");
agents.push(`1. **\`Authorization: Bearer public\`** — pre-issued zero-friction token. No registration or token endpoint walk needed; works immediately.`);
agents.push(`2. **A JWT from \`${SITE}/oauth/token\`** — full \`client_credentials\` flow. \`POST\` with \`grant_type=client_credentials&client_id=public\`; returns a signed JWT (EdDSA when \`SIGNING_PRIVATE_KEY\` is set, HS256 otherwise).`);
agents.push(`3. **Any non-empty Bearer token** — the server semantically remains zero-auth; the requirement is for OAuth-discovery conformance (RFC 8414, RFC 9728, PKCE S256), not access control.`);
agents.push("");
agents.push(`Discovery endpoints (\`GET ${SITE}/mcp\`, \`${SITE}/.well-known/mcp\`, \`${SITE}/.well-known/mcp/server-card.json\`) remain public — no token required for inspection. All other API endpoints (\`/api/*\`, \`/ask\`, \`/status\`, \`/episodes.json\`, \`/rss.xml\`, …) also remain zero-auth.`);
agents.push("");
